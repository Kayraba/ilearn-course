...
export function buildTimeline(i) {
  ...
  const n = i + 1;
  const beats = [];
  const push = (b) => { b.dur = b.dur || sayMs(b.say); b.audioUrl = b.audioUrl || audioFor(n, beats.length); beats.push(b); };
  const pad = String(n).padStart(2, '0');

  push({ kind: 'title', say: `Lesson ${n}. ${L.title}.`, html: titleCard(n, L.title), dur: 4200 });
  push({ kind: 'intro', say: L.tutor, html: outcomeCard(L.outcome), audioUrl: `audio/lesson-${pad}.mp3`, dur: sayMs(L.tutor) });

  // teaching demonstration frames
  F[i].forEach((fr) => push({ kind: 'demo', say: fr.say, html: fr.html, focus: fr.focus, type: fr.type }));

  // common mistake
  push({ kind: 'mistake', say: `Here is a common mix-up to watch for. ${MISTAKES[i]}`, html: mistakeCard(MISTAKES[i]) });

  // closing — a message specific to this lesson, then send them to the activities
  const C = CLOSINGS[i];
  push({ kind: 'done', say: `${C.say} Now scroll down and complete the activities with your tutor — they build on exactly what we have just learned. Work through them at your own pace.`, html: doneCard(C.head) });
