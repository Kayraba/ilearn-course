...
/* per-lesson lead-in to the mistake tip — varied so no line repeats across lessons */
const MISTAKE_LEADS = [
  'One thing to watch for as you start.',
  'A quick tip before you have a go.',
  'Here is where people often slip.',
  'Watch out for this one.',
  'A small thing that is easy to muddle.',
  'Do be careful with this part.',
  'Keep this clear in your mind.',
  'A gentle reminder before you try.',
  'One thing worth remembering.',
  'Good news — this one is hard to get wrong.',
  'This is the step people forget.',
  'One little thing makes all the difference here.',
  'Nothing to worry about, just so you know.',
  'Hold on to this simple idea.',
  'A quick heads-up before you build one.',
  'And the one thing every formula needs.',
];
/* per-lesson closing invitation to the activities — varied, and matched to the skill */
const CLOSING_NEXT = [
  'When you are ready, the activities below let you do it for real.',
  'Have a go at the activities underneath, in your own time.',
  'The tasks below give you a chance to practise this yourself.',
  'Scroll down whenever you are ready to try typing.',
  'The activities below let you open and close a window yourself.',
  'Try saving your own note in the activities below.',
  'The tasks below let you make a folder of your own.',
  'Take the activities below at whatever pace suits you.',
  'The activities underneath let you write your own sentence.',
  'Give the editing tasks below a try when you feel ready.',
  'The activities below let you format your own words.',
  'Have a go at making your own title in the tasks below.',
  'The activities below help you build a real list of your own.',
  'The tasks below let you explore the grid for yourself.',
  'Build your own table in the activities below.',
  'Finish with the activities below, and your certificate is waiting.',
];
...
function audioFor(lessonNo, beatIdx) {
  ...
  if (NARR.base) return `${NARR.base}/lesson-${lessonNo}-${String(beatIdx + 1).padStart(2, '0')}.${NARR.ext || 'mp3'}`;
  return null;
}
...
export function buildTimeline(i) {
  ...
  const n = i + 1;
  const beats = [];
  const push = (b) => { b.dur = b.dur || sayMs(b.say); beats.push(b); };

  push({ kind: 'title', say: `Lesson ${n}. ${L.title}.`, html: titleCard(n, L.title), dur: 4200 });
  ...
  // common mistake — varied lead-in per lesson
  push({ kind: 'mistake', say: `${MISTAKE_LEADS[i]} ${MISTAKES[i]}`, html: mistakeCard(MISTAKES[i]) });

  // closing — a message specific to this lesson, then a varied nudge to the activities
  const C = CLOSINGS[i];
  push({ kind: 'done', say: `${C.say} ${CLOSING_NEXT[i]}`, html: doneCard(C.head) });
